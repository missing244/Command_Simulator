...
DataType = Union[
    "LocalPlayer", "OnlinePlayer", "Village", "Map", "Portal", "Mobevent", "SchedulerWT",
    "Scoreboard", "Structure", "TickingArea"
]
...
class LocalPlayer :
    ...
    * 可用方法 to_leveldb : 将数据写入leveldb对象中
    ...
    def __init__(self, player:TAG_Compound) -> None:
        self.player = player
    ...
    def from_leveldb(cls, leveldb:leveldb.LevelDB) :
        ...
        player = read_from_nbt_file(value, byteorder="little")
        return cls(player)
    
    def to_leveldb(self, leveldb:leveldb.LevelDB) :
        buffer = io.BytesIO(b"")
        write_to_nbt_file(buffer, self.player, byteorder="little")
...
class OnlinePlayer :
    ...
    * 可用方法 to_leveldb : 将数据写入leveldb对象中
    ...
    def __init__(self) -> None:
        self.players:Dict[str, TAG_Compound] = {}
    ...
    def from_leveldb(cls, leveldb:leveldb.LevelDB) :
        ...
        for key in leveldb.keys() :
            ...
            Object[key.decode("utf-8")] = read_from_nbt_file(value, byteorder="little")
        return Object
    ...
    * 可用方法 to_leveldb : 将数据写入leveldb对象中
    ...
    def __init__(self) -> None:
        self.villages:Dict[str, Dict[Literal["dimension", "DWELLERS", "INFO", "PLAYERS", "POI"], 
            Union[str, TAG_Compound, TAG_Compound, TAG_Compound, TAG_Compound]]] = {}
    ...
    def from_leveldb(cls, leveldb:leveldb.LevelDB) :
        ...
        for i in leveldb.keys() :
            ...
            if data_name not in Object.villages[uuid] : 
                Object.villages[uuid][data_name] = read_from_nbt_file(leveldb.get(i), byteorder="little")
    ...
    def to_leveldb(self, leveldb:leveldb.LevelDB) :
        for uuid, value_1 in self.villages.items() :
            for data_name, value_2 in value_1.items() :
                ...
                buffer = io.BytesIO(b"")
                write_to_nbt_file(buffer, value_2, byteorder="little")
...
class Map :
    ...
    * 可用方法 to_leveldb : 将数据写入leveldb对象中
    ...
    def __init__(self) -> None:
        self.maps:Dict[str, TAG_Compound] = {}
    ...
    def from_leveldb(cls, leveldb:leveldb.LevelDB) :
        ...
        for i in leveldb.keys() :
            ...
            Object.maps[i.decode("utf-8")] = read_from_nbt_file(leveldb.get(i), byteorder="little")
...
class Portal :
    ...
    * 可用方法 to_leveldb : 将数据写入leveldb对象中
    ...
    def from_leveldb(cls, leveldb:leveldb.LevelDB) :
        ...
        nbt = read_from_nbt_file(value, byteorder="little")

        return cls(nbt)

    def to_leveldb(self, leveldb:leveldb.LevelDB) :
        buffer = io.BytesIO(b"")
        write_to_nbt_file(buffer, self.nbt, byteorder="little")
...
class Structure :
    ...
    * 可用方法 to_leveldb : 将数据写入leveldb对象中
    ...
    def __init__(self) -> None:
        self.structures:Dict[str, TAG_Compound] = {}
...
class TickingArea :
    ...
    * 可用方法 to_leveldb : 将数据写入leveldb对象中
    ...
    def __init__(self) -> None:
        self.tickingareas:Dict[str, TAG_Compound] = {}
    ...
    def from_leveldb(cls, leveldb:leveldb.LevelDB) :
        ...
        for i in leveldb.keys() :
            ...
            str1 = i.decode("utf-8").split("_", 1)
            Object.tickingareas[str1[1]] = read_from_nbt_file(leveldb.get(i), byteorder="little")
