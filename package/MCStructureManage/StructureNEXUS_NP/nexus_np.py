...
class NexusNP :
    ...
    NexusNP 结构文件对象
    -----------------------
    * 管理 .np(nexus_np) 为后缀的 msgpack 结构文件
    ...
    * 可用方法 save_as : 通过路径或缓冲区保存对象数据
    ...
    def verify(cls, buffer:Union[str, bytes, FileIO, BytesIO]) -> bool :
        ...
        if not isinstance(obj[0][0], list) : return False
        if len(obj[0][0]) != 5 : return False
        return True
    ...
    def from_buffer(cls, buffer:Union[str, bytes, FileIO, BytesIO]) :
        ...
        if not has_pos : raise ValueError("NexusNP文件缺少坐标数据")
        ...
        for block in blocks :
            if not isinstance(block, list) or len(block) < 5 : continue
            name = cls._normalize_name(block[0] if len(block) >= 1 else "minecraft:air")
            if cls._is_air_name(name) : continue

            x = cls._safe_int(block[1])
            y = cls._safe_int(block[2])
            z = cls._safe_int(block[3])
            data_value = cls._safe_int(block[4], 0)
    ...
    def save_as(self, buffer:Union[str, FileIO, BytesIO]) :
        ...
        for index, palette_id in enumerate(self.block_index) :
            ...
            blocks.append([name, int(x), int(y), int(z), int(data_value)])

        if not blocks :
            blocks.append(["minecraft:air", 0, 0, 0, 0])
        corner = [size_x - 1, size_y - 1, size_z - 1]
        if corner != [0, 0, 0] :
            blocks.append(["minecraft:air", corner[0], corner[1], corner[2], 0])
