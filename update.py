...
import tarfile,json,traceback,time,os,re,ssl,random,webbrowser,base64,zlib
        ...
        for times1 in range(2) :
            ...
            try : response2:bytes = request.urlopen(req1,timeout=3).read()
                ...
                if (b'<script type="text/javascript" src="/aes.js" ></script>' in response2) :
                    user_headers["cookie"] = js_execute.decode(response2.decode("utf-8")) ; time.sleep(0.25)
...
print('正在获取更新信息.....')
try : 
    response1:bytes = request_url_without_error("https://sharechain.qq.com/a28ff773d6d07e3830dc7bf54cc7a079")
    if response1 == None : raise Exception
    online_msg = transfor_qq_share(response1.decode("utf-8"))
except : 
    traceback.print_exc()
    print("更新失败，无法获取更新信息")
    time.sleep(10) ; exit()

online_msg = json.loads( zlib.decompress( base64.b64decode( online_msg.encode("utf-8") ) ) )
print("你将要更新：%s" % online_msg['app_version'])
download_url_list = online_msg['app_download']['online_url']
for index1,url2 in enumerate(download_url_list) :
    response2:bytes = request_url_without_error(url2)
    if response2 == None : continue
    break
if response2 == None : 
    print("\n".join(["",
        "########################################################",
    ...
    for i in range(8,0,-1) : print("%s秒后即将打开浏览器......" % i) ; time.sleep(1)
    webbrowser.open(online_msg['app_download']['offline_url'])
    exit()
else :
